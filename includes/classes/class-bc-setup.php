...
class BC_Setup {
	...
	public static function add_brightcove_media_button() {
		...
		echo '<a href="#" id="brightcove-add-media" class="button brightcove-add-media"><img class="bc-button-icon" src="' . esc_url( BRIGHTCOVE_URL . 'images/admin/menu-icon.svg' ) . '"> ' . __( 'Brightcove Media', 'brightcove' ) . '</a>';
	}
	...
	public static function preload_params() {
		...
		$params['messages'] = array(
			'confirmDelete' => __( 'Deleting this video will prevent it from showing in any existing posts. Are you sure you want to delete?', 'brightcove' ),
			'ongoingSync' => __( 'We are currently performing a sync of your new Brightcove source, you may not see all videos and playlists until that is complete.', 'brightcove' ),
			'successUpload' => __( 'Successfully uploaded file with name %%s%%.', 'brightcove' ),
			'unableToUpload' => __( 'We were unable to upload the file with name %%s%% Please try reuploading it again.', 'brightcove' ),
		);
	...
	public static function admin_enqueue_scripts() {
		...
		wp_register_script( 'require-js', esc_url( BRIGHTCOVE_URL . 'assets/js/vendor/require.min.js' ) );
	...
	public static function bc_activation_admin_notices() {
		...
		if ( get_option( '_brightcove_plugin_activated' ) !== false
			...
			$notices[] = array(
				'message' => sprintf( __( 'Please configure Brightcove settings from <a href="%s"><strong>here</strong></a>.', 'brightcove' ),
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.9.2', '<=' ) ) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				function bc_plugin_incompatible_admin_notice() {
					...
					echo sprintf( __( '<div class="error"><p><strong>Brightcove Video Cloud Enhanced</strong> has been <strong>deactivated</strong> because it\'s incompatibale with WordPress version %s! The minimum compatible WordPress version is <strong>4.0</strong></p></div>', 'brightcove' ), esc_html( get_bloginfo( 'version' ) ) );
